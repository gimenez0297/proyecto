...
$fileName = "productosVendidosPorSucursal.xls";
...
foreach ($productos as $r) {
    ...
    foreach ($sucursales as $v) {
        ...
        if (in_array($v->id_sucursal, $columnas)) {
            $productos_vendidos->setCellValueByColumnAndRow(array_search($v->id_sucursal, $columnas)+1, $coun, $cantidad);
...
$productos_vendidos->getStyle($coun)->getFont()->setBold(true);
 ...
 //$productos_vendidos->setCellValueByColumnAndRow(count($columnas) - 1, $coun, $total);

foreach ($sucursales as $v) {
    $total = $totales[$v->id_sucursal];

    if (in_array($v->id_sucursal, $columnas)) {
        $productos_vendidos->setCellValueByColumnAndRow(array_search($v->id_sucursal, $columnas)+1, $coun, $total);
    }
}
