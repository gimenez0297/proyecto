var url = 'inc/caja-chica-configuracion-data';
...
$('#id_responsable').select2({
    ...
    ajax: {
        ...
        async: true,
        data: function (params) {
            return { q: 'usuarios_encargados_caja_chica', term: params.term, page: params.page || 1, id: $('#id_sucursal').val() }
        },
        processResults: function (data, params) {
            params.page = params.page || 1;
            return {
        ...
        sweetAlertConfirm({
            ...
            confirm: function() {
                $.ajax({
                    ...
                    type: 'post',
                    data: { id: row.id_caja_chica, estado },
                    beforeSend: function() {
                        NProgress.start();
                    },
...
$("#tabla").bootstrapTable({
    ...
    height: alturaTabla(),
    pageSize: pageSizeTabla(),
    sortName: "id_caja_chica",
    sortOrder: 'desc',
    trimOnSearch: false,
    columns: [
        [
            { field: 'id_caja_chica', title: 'ID Doctor', align: 'left', valign: 'middle', sortable: true, visible: false },
            { field: 'sucursal', title: 'Sucursal', align: 'left', valign: 'middle', sortable: true },
            { field: 'funcionario', title: 'Responsable', align: 'left', valign: 'middle', sortable: true, cellStyle: bstTruncarColumna },
            { field: 'monto', title: 'Monto', align: 'right', valign: 'middle', sortable: true, formatter: separadorMiles},
            { field: 'monto_minimo', title: 'Monto Mínimo', align: 'right', valign: 'middle', sortable: true, formatter: separadorMiles },
            { field: 'maximo_factura', title: 'Monto Máximo  De Factura', align: 'right', valign: 'middle', sortable: true, formatter: separadorMiles,width: 200  },
...
$('#agregar').click(function() {
    $('#modalLabel').html('Agregar Caja Chica');
...
$('#eliminar').click(function() {
    var id = $("#id").val();
    var id_sucursal = $('#id_sucursal').select2('data')[0];
    var sucursal = id_sucursal.text;

    sweetAlertConfirm({
        title: `Eliminar`,
        text: `¿Eliminar la Caja Chica de ${sucursal}?`,
        closeOnConfirm: false,
        confirm: function() {
            $.ajax({
                dataType: 'json',
...
function editarDatos(row) {
    resetForm('#formulario');
    $('#modalLabel').html('Editar Caja Chica');
    $('#formulario').attr('action', 'editar');
    $('#eliminar').show();
    $('#modal').modal('show');

    $("#id").val(row.id_caja_chica);
    ...
    $("#minimo").val(separadorMiles(row.monto_minimo));
    $("#max_factura").val(separadorMiles(row.maximo_factura));
    if (row.id_funcionario > 0) {
        $('#id_responsable').select2('trigger', 'select', {
            data: { id: row.id_funcionario, text: row.funcionario}
        });
    }
}