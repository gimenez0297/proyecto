var url = 'inc/administrar-compras-data';

function iconosFila(value, row, index) {
    // Pendiente y Aprobado, los demas estado disabled
    let disabled = (row.estado == 0 || row.estado == 1) ? '' : 'disabled';
...
window.accionesFila = {
    'click .ver-productos': function(e, value, row, index) {
        ...
        $('#toolbar_productos_text').html(`Orden de Compra N° ${row.numero}`);
        $('#tabla_productos').bootstrapTable('refresh', { url: url+'?q=ver_productos&id_orden_compra='+row.id_orden_compra });
    },
        ...
        sweetAlertConfirm({
            ...
            confirm: function() {
                $.ajax({
                    ...
                    type: 'post',
                    data: { id: row.id_orden_compra, estado },
                    beforeSend: function() {
                        NProgress.start();
                    },
    ...
    'click .imprimir': function (e, value, row, index) {
        var param = { id_orden_compra: row.id_orden_compra};
        OpenWindowWithPost("imprimir-compras", "toolbar=0,scrollbars=1,location=0,statusbar=0,menubar=1,resizable=yes,width=1324,height=728", "ImprimirCompras", param);
    }
}
...
$("#tabla").bootstrapTable({
    ...
    columns: [
        [
            ...
            { field: 'id_orden_compra', title: 'ID', align: 'left', valign: 'middle', sortable: true, visible: false },
...
function toggleButons(value, row, index, field) {
    ...
    $('#btn_aprobar, #btn_rechazar').prop('disabled', (selections.length > 0) ? false : true);
}
...
$('#btn_rechazar').on('click', function() {
    cambiarEstado(2, 'Rechazado', 'Rechazar', 'danger');
});
...
$("#tabla_productos").bootstrapTable({
    ...
    columns: [
        [
            ...
            { field: 'id_solicitud_compra', title: 'ID SOLICITUD COMPRA', align: 'left', valign: 'middle', sortable: true, visible: false },
            { field: 'numero', title: 'N° SOLICITUD', align: 'left', valign: 'middle', sortable: true, visible: true, footerFormatter: bstFooterTextTotal },
            ...
            { field: 'porc_desc', title: 'DESC. %', align: 'right', valign: 'middle', sortable: true, formatter: separadorMiles},
            { field: 'precio_venta', title: 'P.PÚBLICO', align: 'right', valign: 'middle', sortable: true, formatter: separadorMiles},
            ...
            { field: 'total_recepcionado', title: 'TOTAL RECEP.', align: 'right', valign: 'middle', sortable: true, formatter: separadorMiles, footerFormatter: bstFooterSumatoria  },
        ]
    ]
});
