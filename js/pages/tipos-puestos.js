var url = 'inc/tipos-puestos-data';
...
$('#tabla').bootstrapTable({
    ...
    height: alturaTabla(),
    pageSize: pageSizeTabla(),
    sortName: 'tipo_puesto',
    sortOrder: 'asc',
    trimOnSearch: false,
    columns: [
        [
            { field: 'id_tipo_puesto', title: 'ID', align: 'left', valign: 'middle', sortable: true, width: 200 },
            { field: 'tipo_puesto', title: 'Tipo', align: 'left', valign: 'middle', sortable: true, cellStyle: bstTruncarColumna },
...
function editarDatos(row) {
    ...
    $('#modalLabel').html('Editar Tipo De Puesto');
    $('#formulario').attr('action', 'editar');
    $('#modal').modal('show');

    $('#id').val(row.id_tipo_puesto);
    $('#tipo_puesto').val(row.tipo_puesto);
}
