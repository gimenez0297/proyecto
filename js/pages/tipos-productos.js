var url = 'inc/tipos-productos-data';
        ...
        sweetAlertConfirm({
            ...
            confirm: function() {
                $.ajax({
                    ...
                    type: 'post',
                    data: { id_tipo_producto: row.id_tipo_producto, estado },
                    beforeSend: function() {
                        NProgress.start();
                    },
...
$("#tabla").bootstrapTable({
    ...
    columns: [
        [
            { field: 'id_tipo_producto', title: 'ID Tipo De Producto', align: 'left', valign: 'middle', sortable: true, visible: false },
            { field: 'tipo', title: 'Tipo', align: 'left', valign: 'middle', sortable: true, cellStyle: bstTruncarColumna },
            { field: 'principios_activos_str', title: 'Principios Activos', align: 'center', valign: 'middle', sortable: true, width: 200 },
            { field: 'lote_automatico_str', title: 'Lote automático', align: 'center', valign: 'middle', sortable: true, width: 200 },
            { field: 'lote_prefijo', title: 'Prefijo Lote', align: 'center', valign: 'middle', sortable: true, width: 200 },
...
$('#agregar').click(function() {
    $('#modalLabel').html('Agregar Tipo De Producto');
...
$('#eliminar').click(function() {
    var id = $("#id_tipo_producto").val();
    var nombre = $("#tipo").val();


    sweetAlertConfirm({
        title: `Eliminar`,
        text: `¿Eliminar Tipo De Producto: ${nombre}?`,
        closeOnConfirm: false,
        confirm: function() {
            $.ajax({
                dataType: 'json',
...
function editarDatos(row) {
    resetForm('#formulario');
    $('#modalLabel').html('Editar Tipo De Producto');
    $('#formulario').attr('action', 'editar');
    $('#eliminar').show();
    $('#modal').modal('show');

    $("#id_tipo_producto").val(row.id_tipo_producto);
    $("#tipo").val(row.tipo);
    if (row.principios_activos == 1) {
        $("#principios_activos").prop('checked', true).trigger('change');
    }
    if (row.lote_automatico == 1) {
        $("#lote_automatico").prop('checked', true).trigger('change');
        $("#lote_prefijo").val(row.lote_prefijo);
    }
}
