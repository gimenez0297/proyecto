var url = 'inc/clasificacion-productos-data';
        ...
        sweetAlertConfirm({
            ...
            confirm: function() {
                $.ajax({
                    ...
                    type: 'post',
                    data: { id_clasificacion_producto: row.id_clasificacion_producto, estado },
                    beforeSend: function() {
                        NProgress.start();
                    },
...
$("#tabla").bootstrapTable({
    ...
    sortName: "clasificacion",
    sortOrder: 'asc',
    trimOnSearch: false,
    columns: [
        [
            { field: 'id_clasificacion_producto', title: 'ID Clasificación De Producto', align: 'left', valign: 'middle', sortable: true, visible: false },
            { field: 'clasificacion', title: 'Clasificación', align: 'left', valign: 'middle', sortable: true, cellStyle: bstTruncarColumna },
            { field: 'id_grupo', title: 'ID Grupo', align: 'left', valign: 'middle', sortable: true, visible: false },
            { field: 'grupo', title: 'Grupo', align: 'left', valign: 'middle', sortable: true, cellStyle: bstTruncarColumna },
...
$('#agregar').click(function() {
    $('#modalLabel').html('Agregar Clasificación De Producto');
...
$('#eliminar').click(function() {
    var id = $("#id_clasificacion_producto").val();
    var nombre = $("#clasificacion").val();

    sweetAlertConfirm({
        title: `Eliminar`,
        text: `¿Eliminar Clasificación De Producto: ${nombre}?`,
        closeOnConfirm: false,
        confirm: function() {
            $.ajax({
                dataType: 'json',
...
function editarDatos(row) {
    resetForm('#formulario');
    $('#modalLabel').html('Editar Clasificación');
    $('#formulario').attr('action', 'editar');
    $('#eliminar').show();
    $('#modal').modal('show');

    $("#id_clasificacion_producto").val(row.id_clasificacion_producto);
    $("#clasificacion").val(row.clasificacion);
    if (row.id_grupo) {
        $("#grupo").append(new Option(row.grupo, row.id_grupo, true, true)).trigger('change');
    }
}
...
$('#grupo').select2({
    ...
    ajax: {
        ...
        data: function(params) {
            return { q: 'grupos_clasificaciones', term: params.term, page: params.page || 1 }
        },
        processResults: function(data, params) {
            params.page = params.page || 1;
            return {
                results: $.map(data.data, function(obj) { return { id: obj.id_grupo, text: obj.grupo }; }),
