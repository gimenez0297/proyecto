var url = 'inc/tipos-insumos-data';
        ...
        sweetAlertConfirm({
            ...
            confirm: function() {
                $.ajax({
                    ...
                    type: 'post',
                    data: { id_tipo_insumo: row.id_tipo_insumo, estado },
                    beforeSend: function() {
                        NProgress.start();
                    },
...
$("#tabla").bootstrapTable({
    ...
    columns: [
        [
            { field: 'id_tipo_insumo', title: 'ID Tipo De Insumo', align: 'left', valign: 'middle', sortable: true, visible: false },
            { field: 'nombre', title: 'Nombre', align: 'left', valign: 'middle', sortable: true, cellStyle: bstTruncarColumna },
...
$('#agregar').click(function() {
    $('#modalLabel').html('Agregar Tipo De Insumo');
...
$('#eliminar').click(function() {
    ...
    sweetAlertConfirm({
        title: `Eliminar`,
        text: `¿Eliminar Tipo De Insumo: ${nombre}?`,
        closeOnConfirm: false,
        confirm: function() {
            $.ajax({
                dataType: 'json',
...
function editarDatos(row) {
    resetForm('#formulario');
    $('#modalLabel').html('Editar Tipo De Insumo');
    $('#formulario').attr('action', 'editar');
    $('#eliminar').show();
    $('#modal').modal('show');

    $("#id_tipo_insumo").val(row.id_tipo_insumo);
    $("#nombre").val(row.nombre);
    if (row.nombre == 1) {
        $("#nombre").prop('checked', true).trigger('change');
    }
}
