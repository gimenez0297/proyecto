        ...
        sweetAlertConfirm({
            ...
            confirm: function() {
                $.ajax({
                    ...
                    type: 'post',
                    data: { id: row.id_delivery, estado },
                    beforeSend: function() {
                        NProgress.start();
                    },
...
$("#tabla").bootstrapTable({
    ...
    height: alturaTabla(),
    pageSize: pageSizeTabla(),
    sortName: "id_delivery",
    sortOrder: 'asc',
    trimOnSearch: false,
    columns: [
        [
            { field: 'id_delivery', title: 'ID Delivery', align: 'left', valign: 'middle', sortable: true, visible: false },
            { field: 'ciudad', title: 'Ciudad', align: 'left', valign: 'middle', sortable: true, cellStyle: bstTruncarColumna },
...
$('#agregar').click(function() {
    $('#modalLabel').html('Agregar Tarifa de Delivery');
...
function editarDatos(row) {
    resetForm('#formulario');
    $('#modalLabel').html('Editar Tarifa de Delivery');
    $('#formulario').attr('action', 'editar');
    $('#eliminar').show();
    $('#modal').modal('show');

    $("#id").val(row.id_delivery);
    $("#precio").val(separadorMiles(row.precio));

    if (row.id_distrito > 0) {
        $('#id_distrito').select2('trigger', 'select', {
            data: { id: row.id_distrito, text: row.ciudad}
        });
    }
}
