var url = 'inc/unidades-medidas-data';
        ...
        sweetAlertConfirm({
            ...
            confirm: function() {
                $.ajax({
                    ...
                    type: 'post',
                    data: { id_unidad_medida: row.id_unidad_medida, estado },
                    beforeSend: function() {
                        NProgress.start();
                    },
...
$("#tabla").bootstrapTable({
    ...
    pageSize: Math.floor(($(window).height()-120)/43),
    sortName: "unidad_medida",
    sortOrder: 'asc',
    trimOnSearch: false,
    columns: [
        [
            { field: 'id_unidad_medida', title: 'ID Unidad De Medida', align: 'left', valign: 'middle', sortable: true, visible: false },
            { field: 'unidad_medida', title: 'Unidad De Medida', align: 'left', valign: 'middle', sortable: true, cellStyle: bstTruncarColumna },
            { field: 'sigla', title: 'Sigla', align: 'left', valign: 'middle', sortable: true, cellStyle: bstTruncarColumna },
...
$('#agregar').click(function() {
    $('#modalLabel').html('Agregar Unidad De Medida');
...
$('#eliminar').click(function() {
    ...
    var nombre = $("#unidad_medida").val();

    sweetAlertConfirm({
        title: `Eliminar`,
        text: `¿Eliminar Unidad De Medida: ${nombre}?`,
        closeOnConfirm: false,
        confirm: function() {
            $.ajax({
                dataType: 'json',
...
function editarDatos(row) {
    resetForm('#formulario');
    $('#modalLabel').html('Editar Unidad De Medida');
    $('#formulario').attr('action', 'editar');
    $('#eliminar').show();
    $('#modal').modal('show');

    $("#id_unidad_medida").val(row.id_unidad_medida);
    $("#unidad_medida").val(row.unidad_medida);
    $("#sigla").val(row.sigla);
}
