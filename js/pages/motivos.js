        ...
        sweetAlertConfirm({
            ...
            confirm: function() {
                $.ajax({
                    ...
                    type: 'post',
                    data: { id_motivo: row.id_nota_remision_motivo, estado },
                    beforeSend: function() {
                        NProgress.start();
                    },
...
$("#tabla").bootstrapTable({
    ...
    sortName: "id_nota_remision_motivo",
    sortOrder: 'asc',
    trimOnSearch: false,
    columns: [
        [
            { field: 'id_nota_remision_motivo', title: 'ID Motivo', align: 'left', valign: 'middle', sortable: true, visible: false },
            { field: 'descripcion', title: 'Descripcion', align: 'left', valign: 'middle', sortable: true, cellStyle: bstTruncarColumna },
            { field: 'nombre_corto', title: 'Nombre Corto', align: 'left', valign: 'middle', sortable: true, cellStyle: bstTruncarColumna },
...
$('#agregar').click(function() {
    $('#modalLabel').html('Agregar Motivo');
...
$('#eliminar').click(function() {
    ...
    var motivo = $("#nombre_corto").val();

    sweetAlertConfirm({
        title: `Eliminar`,
        text: `¿Eliminar Motivo: ${motivo}?`,
        closeOnConfirm: false,
        confirm: function() {
            $.ajax({
                dataType: 'json',
...
function editarDatos(row) {
    resetForm('#formulario');
    $('#modalLabel').html('Editar Motivo');
    $('#formulario').attr('action', 'editar');
    $('#eliminar').show();
    $('#modal').modal('show');

    $("#descripcion").val(row.descripcion);
    $("#nombre_corto").val(row.nombre_corto);
    $("#id_motivo").val(row.id_nota_remision_motivo);

    if (row.id_nota_remision_motivo == 1) {
        $('#eliminar').hide();
    }else{
        $('#eliminar').show();
    }
