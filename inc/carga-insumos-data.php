<?php
    ...
    switch ($q) {
        ...
        case 'cargar':
            ...
            if (empty($producto)) {echo json_encode(["status" => "error", "mensaje" => "Ningún producto agregado. Favor verifique."]);exit;}
            ...
            $db->setQuery("SELECT MAX(numero) AS numero FROM cargas_insumos");
            $row = $db->loadObject();
            ...
            $numero_sgte = zerofill($sgte_numero);
             ...
             // Se calcula el total monto y la cantidad total de productos
            $total_cantidad=0;
            $total_costo=0;
            ...
            // Cabecera
            $db->setQuery("INSERT INTO cargas_insumos (numero, cantidad, monto, observacion, estado, usuario, fecha) 
            ...
            $id_carga_insumo = $db->getLastID();

            //Tipo de Gasto
            $db->setQuery("SELECT * FROM tipos_gastos WHERE id_sub_tipo_gasto = 2");
            $gasto = $db->loadObject();
            if (!$gasto) {
                echo json_encode(["status" => "error", "mensaje" => "No hay un gasto cargado con tipo Carga de Insumos"]);
                exit;
            }
            ...
            //Datos de la Factura
            $db->setQuery("INSERT INTO gastos (id_tipo_gasto,id_sucursal,id_tipo_comprobante,id_sub_tipo_gasto,id_proveedor,id_carga_insumo,tipo_proveedor,nro_gasto,timbrado,fecha_emision,ruc,razon_social,condicion,
                           ...
                           VALUES($id_tipo_gasto,$id_sucursal,$id_tipo_comprobante,$id_sub_tipo_gasto,$id_proveedor,$id_carga_insumo,2,'$nro_gasto','$timbrado','$fecha_emision','$ruc','$razon_social',$condicion,
            ...
            foreach ($vencimientos as $key => $value) {
             ...
             foreach ($producto as $p) {
                ...
                $id_producto_insumo = $db->clearText($p['id_producto_insumo']);
                ...
                $db->setQuery("UPDATE productos_insumo  SET costo=$monto WHERE id_producto_insumo=$id_producto_insumo");
                if (!$db->alter()) {
                    echo json_encode(["status" => "error", "mensaje" => "Error. ".$db->getError()]);
                    $db->rollback();  // Revertimos los cambios
                    exit;
             ...
             $db->setQuery("INSERT INTO cargas_insumos_productos (id_carga_insumo, id_producto_insumo, cantidad, vencimiento, monto) 
                                        VALUES ('$id_carga_insumo', '$id_producto_insumo', '$cantidad',$vencimiento_null, $monto)");
                if (!$db->alter()) {
                    echo json_encode(["status" => "error", "mensaje" => "Error. ".$db->getError()]);
                    $db->rollback();  // Revertimos los cambios
                    exit;
                ...
                producto_insumo_sumar_stock($db, $id_producto_insumo, $vencimiento, $cantidad);
                ...
                $historial->id_origen = $id_carga_insumo;
                $historial->origen = CAR;
                $historial->detalles = "Recepción N° " .$numero_sgte;
            ...
            echo json_encode(["status" => "ok", "mensaje" => "Carga de Insumo registrada correctamente"]);
        break;
